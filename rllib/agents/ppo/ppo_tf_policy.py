...
def ppo_surrogate_loss(
    ...
    policy.curr_dist_params = curr_action_dist.inputs
    policy.prev_dist_params = prev_action_dist.inputs

    policy.curr_actions_logp = curr_action_dist.logp(train_batch[SampleBatch.ACTIONS])

    policy.logp_ratio = tf.exp(
        policy.curr_actions_logp -
    ...
    policy.surrogate_loss = tf.minimum(
        train_batch[Postprocessing.ADVANTAGES] * policy.logp_ratio,
        train_batch[Postprocessing.ADVANTAGES] * tf.clip_by_value(
            policy.logp_ratio, 1 - policy.config["clip_param"],
            1 + policy.config["clip_param"]))
    mean_policy_loss = reduce_mean_valid(-policy.surrogate_loss)
    ...
    if policy.config["use_gae"]:
        ...
        total_loss = reduce_mean_valid(
            -policy.surrogate_loss + policy.kl_coeff * action_kl +
            policy.config["vf_loss_coeff"] * vf_loss -
            policy.entropy_coeff * curr_entropy)
    else:
        mean_vf_loss = tf.constant(0.0)
        total_loss = reduce_mean_valid(-policy.surrogate_loss +
