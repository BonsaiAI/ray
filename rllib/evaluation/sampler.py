...
class _EnvRunner(Iterator[SampleBatch]):
    def __init__(self,
        ...
        self.observation_fn = observation_fn
        self._use_trajectory_view_api = _use_trajectory_view_api
        self._sample_collector = _sample_collector
        ...
        if self.horizon:
            ...
            if max_episode_steps and self.horizon > max_episode_steps:
                raise ValueError(
    ...
    def _process_observations_w_trajectory_view_api(self,
        ...
        for env_id, all_agents_obs in unfiltered_obs.items():
            ...
            if not is_new_episode:
                self._sample_collector.episode_step(episode.episode_id)
                episode._add_agent_rewards(rewards[env_id])
            ...
            for agent_id, raw_obs in all_agents_obs.items():
                ...
                if last_observation is None:
                    self._sample_collector.add_init_obs(episode, agent_id, env_id,
                    ...
                    self._sample_collector.add_action_reward_next_obs(
            ...
            if all_agents_done:
                ...
                # episodes.
                ma_sample_batch = self._sample_collector.postprocess_episode(
                    episode,
                ...
                # If reset is async, we will get its result in some future poll.
                    ...
                    for agent_id, raw_obs in resetted_obs.items():
                        ...
                        self._sample_collector.add_init_obs(
        ...
        if self.multiple_episodes_in_batch:
            sample_batches = \
                self._sample_collector.try_build_truncated_episode_multi_agent_batch()
            if sample_batches:
    ...
    def _do_policy_eval_w_trajectory_view_api(self,
        ...
        for policy_id, eval_data in to_eval.items():
            policy: Policy = _get_or_raise(self.policies, policy_id)
            input_dict = self._sample_collector.get_inference_input_dict(policy_id)
