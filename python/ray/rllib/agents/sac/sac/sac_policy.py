...
def build_action_output(policy, model, input_dict, obs_space, action_space, config):
    ...
    stochastic_actions = (
        squashed_stochastic_actions
        if config["normalize_actions"]
        else unsquash_actions(squashed_stochastic_actions)
    )
    ...
    deterministic_actions = (
        ...
        if config["normalize_actions"]
        else unsquash_actions(squashed_deterministic_actions)
    )

    actions = tf.cond(
        policy.stochastic, lambda: stochastic_actions, lambda: deterministic_actions
    )
...
def actor_critic_loss(policy, model, _, train_batch):
    ...
    target_entropy = (
        -np.prod(policy.action_space.shape)
        if policy.config["target_entropy"] == "auto"
        else policy.config["target_entropy"]
    )
