...
class SyncSampler(SamplerInput):
    ...
    def __init__(
        ...
        self.rollout_provider = _EnvRunner(
...
class _EnvRunner(Iterator[SampleBatch]):
    def __init__(self,
                 worker: "RolloutWorker",
                 base_env: BaseEnv,
        ...
        self.extra_batch_callback = extra_batch_callback
        self.policies = policies
        self.policy_mapping_fn = policy_mapping_fn
        self.rollout_fragment_length = rollout_fragment_length
        self.horizon = horizon
        ...
        try:
            self.max_episode_steps = base_env.get_unwrapped()[0].spec.max_episode_steps
        except Exception:
            pass
        ...
        if self.horizon:
            ...
            if self.max_episode_steps and self.horizon > self.max_episode_steps:
                # Try to override the env's own max-step setting with our horizon.
                # If this won't work, throw an error.
                try:
                    self.base_env.get_unwrapped()[0].spec.max_episode_steps = self.horizon
                    self.base_env.get_unwrapped()[0]._max_episode_steps = self.horizon
                except Exception:
                    raise ValueError(
        ...
        elif self.max_episode_steps:
            self.horizon = self.max_episode_steps
            logger.debug(
                "No episode horizon specified, setting it to Env's limit ({}).".
                format(self.max_episode_steps))
        ...
        # trajectory data.
        self.batch_builder_pool: List[MultiAgentSampleBatchBuilder] = []

        def get_batch_builder():
            if self.batch_builder_pool:
                return self.batch_builder_pool.pop()
        ...
        self.active_episodes: Dict[str, MultiAgentEpisode] = \
    ...
    def __next__(self) -> SampleBatch:
        while not self.outputs:
            ...
            unfiltered_obs, rewards, dones, infos, off_policy_actions = \
                self.base_env.poll()
            self.infos = infos
            self.perf_stats.env_wait_time += time.time() - t0
            ...
            if self._use_trajectory_view_api:
                active_envs, to_eval, outputs = \
                    self._process_observations_w_trajectory_view_api(
            ...
            self.perf_stats.raw_obs_processing_time += time.time() - t1
            ...
            if self._use_trajectory_view_api:
                eval_results = self._do_policy_eval_w_trajectory_view_api(
                    to_eval=to_eval,
                )
            else:
                eval_results = self._do_policy_eval(
                    to_eval=to_eval,
                )
            self.perf_stats.inference_time += time.time() - t2
            ...
            self.perf_stats.action_processing_time += time.time() - t3

            # Return computed actions to ready envs. We also send to envs that have
            # taken off-policy actions; those envs are free to ignore the action.
            t4 = time.time()
            self.base_env.send_actions(actions_to_send)
            self.perf_stats.env_wait_time += time.time() - t4
    ...
    def _process_observations(self,
                              *,
                              ...
                              ) -> Tuple[Set[EnvID], Dict[PolicyID, List[PolicyEvalData]], List[Union[RolloutMetrics, SampleBatchType]]]:
        ...
        large_batch_threshold: int = max(1000, self.rollout_fragment_length * 10) if \
            self.rollout_fragment_length != float("inf") else 5000

        # For each environment.
        # type: EnvID, Dict[AgentID, EnvObsType]
        for env_id, agent_obs in unfiltered_obs.items():
            is_new_episode: bool = env_id not in self.active_episodes
            episode: MultiAgentEpisode = self.active_episodes[env_id]
            ...
            if dones[env_id]["__all__"] or episode.length >= self.horizon:
                hit_horizon = (episode.length >= self.horizon
            ...
            if self.observation_fn:
                agent_obs: Dict[AgentID, EnvObsType] = self.observation_fn(
                    agent_obs=agent_obs,
                    worker=self.worker,
            ...
            for agent_id, raw_obs in agent_obs.items():
                ...
                prepr = _get_or_raise(self.preprocessors, policy_id)
                prep_obs: EnvObsType = prepr.transform(raw_obs)
                if log_once("prep_obs"):
                    logger.info("Preprocessed obs: {}".format(summarize(prep_obs)))

                filter = _get_or_raise(self.obs_filters, policy_id)
                ...
                if (last_observation is not None and infos[env_id].get(
                    ...
                    batch_builder.add_values(
                        ...
                        dones=(False if (self.no_done_at_end
                                         or (hit_horizon and self.soft_horizon)) else
                               agent_done),
            ...
            self.callbacks.on_episode_step(
                worker=self.worker,
                base_env=self.base_env,
                episode=episode,
                env_index=env_id)
            ...
            if batch_builder.has_pending_agent_data():
                ...
                # is True.
                if dones[env_id]["__all__"] and not self.no_done_at_end:
                ...
                if (all_agents_done and not self.multiple_episodes_in_batch) or \
                        batch_builder.count >= self.rollout_fragment_length:
            ...
            if all_agents_done:
                ...
                self.batch_builder_pool.append(batch_builder)
                ...
                if hit_horizon and self.soft_horizon:
                    episode.soft_reset()
                ...
                # Try hard env-reset.
                    ...
                    del self.active_episodes[env_id]
                    resetted_obs: Dict[AgentID, EnvObsType] = self.base_env.try_reset(
                        env_id)
                if resetted_obs is None:
                    ...
                    episode: MultiAgentEpisode = self.active_episodes[env_id]
                    if self.observation_fn:
                        resetted_obs: Dict[AgentID, EnvObsType] = self.observation_fn(
                    ...
                    for agent_id, raw_obs in resetted_obs.items():
                        policy_id: PolicyID = episode.policy_for(agent_id)
                        policy: Policy = _get_or_raise(self.policies, policy_id)
                        prep_obs: EnvObsType = _get_or_raise(
                            self.preprocessors, policy_id).transform(raw_obs)
                        filtered_obs: EnvObsType = _get_or_raise(
                            self.obs_filters, policy_id)(prep_obs)
                        episode._set_last_observation(agent_id, filtered_obs)
    ...
    def _process_observations_w_trajectory_view_api(self,
            *,
            unfiltered_obs: Dict[EnvID, Dict[AgentID, EnvObsType]],
            rewards: Dict[EnvID, Dict[AgentID, float]],
            dones: Dict[EnvID, Dict[AgentID, bool]],
        ...
        for env_id, all_agents_obs in unfiltered_obs.items():
            ...
            if not is_new_episode:
                self.sample_collector.episode_step(episode.episode_id)
                episode._add_agent_rewards(rewards[env_id])
            ...
            # Custom observation function is applied before preprocessing.
            if self.observation_fn:
                all_agents_obs: Dict[AgentID, EnvObsType] = self.observation_fn(
            ...
            for agent_id, raw_obs in all_agents_obs.items():
                assert agent_id != "__all__"
                policy_id: PolicyID = episode.policy_for(agent_id)
                prep_obs: EnvObsType = _get_or_raise(self.preprocessors,
                                                     policy_id).transform(raw_obs)
                if log_once("prep_obs"):
                    logger.info("Preprocessed obs: {}".format(summarize(prep_obs)))

                filtered_obs: EnvObsType = _get_or_raise(self.obs_filters,
                                                         policy_id)(prep_obs)
                if log_once("filtered_obs"):
                    logger.info("Filtered obs: {}".format(summarize(filtered_obs)))
                ...
                if last_observation is None:
                    self.sample_collector.add_init_obs(episode, agent_id, env_id,
            ...
            if all_agents_done:
                ...
                check_dones = is_done and not self.no_done_at_end
                ...
                # episodes.
                ma_sample_batch = self.sample_collector.postprocess_episode(
                    episode,
                    is_done=is_done or (hit_horizon and not self.soft_horizon),
                    check_dones=check_dones,
                    build=not self.multiple_episodes_in_batch)
                if ma_sample_batch:
                ...
                # If reset is async, we will get its result in some future poll.
                    ...
                    new_episode: MultiAgentEpisode = self.active_episodes[env_id]
                    if self.observation_fn:
                        resetted_obs: Dict[AgentID, EnvObsType] = self.observation_fn(
                            agent_obs=resetted_obs,
                            worker=self.worker,
                    ...
                    for agent_id, raw_obs in resetted_obs.items():
                        ...
                        self.sample_collector.add_init_obs(
        ...
        if self.multiple_episodes_in_batch:
            sample_batches = \
    ...
    def _do_policy_eval(self,
        ...
        if self.tf_sess:
            builder = TFRunBuilder(self.tf_sess, "policy_eval")
        ...
        for policy_id, eval_data in to_eval.items():
            ...
            if builder and (policy.compute_actions.__code__ is
                ...
                eval_results[policy_id] = policy.compute_actions(
                    ...
                    episodes=[self.active_episodes[t.env_id] for t in eval_data],
                    timestep=policy.global_timestep)
    ...
    def _do_policy_eval_w_trajectory_view_api(self,
            *,
            to_eval: Dict[PolicyID, List[PolicyEvalData]],
    ) -> Dict[PolicyID, Tuple[TensorStructType, StateBatch, dict]]:
        """Call compute_actions on collected episode/model data to get next action.
    ...
    def _process_policy_eval_results(self,
                                     *,
                                     to_eval: Dict[PolicyID, List[PolicyEvalData]],
