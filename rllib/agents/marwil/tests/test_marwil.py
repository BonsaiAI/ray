...
from ray.rllib.utils.test_utils import framework_iterator, check, check_compute_action
...
class TestMARWIL(unittest.TestCase):
    ...
    def test_marwil_loss_function(self):
        ...
        for fw in framework_iterator(config, frameworks=["torch", "tfe"]):
            ...
            if fw == "torch":
                check(policy.v_loss, expected_vf_loss, decimals=3)
                check(policy.p_loss, expected_pol_loss, decimals=3)
            else:
                check(policy.loss.v_loss, expected_vf_loss, decimals=3)
                check(policy.loss.p_loss, expected_pol_loss, decimals=3)
            check(loss_out, expected_loss, decimals=2)
